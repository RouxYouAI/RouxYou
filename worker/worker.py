...
# Must be set before capabilities imports
sys.path.insert(0, str(_PROJECT_ROOT))
sys.path.insert(0, str(_PROJECT_ROOT.parent))

from capabilities.vision import capture_screen_analysis
from capabilities.verify import run_verification
...
from shared.tier0 import TIER0_FILES, violates_tier0, runcmd_hits_tier0
...
class SuperWorker:
    ...
    # Tier-0 deny lives in shared/tier0.py (SINGLE SOURCE OF TRUTH — worker + executor both
    # import it so they can never drift). Gates the AUTONOMOUS path only; Claude-in-loop + DJ
    # edits bypass the worker, so the apex stays evolvable through the highest-trust gate.
    IMMUTABLE_FILES = TIER0_FILES  # re-exported for back-compat / introspection
    ...
    # Tier-0 checks delegate to shared/tier0.py (single source of truth).
    def _violates_tier0(self, raw_target: str) -> "str | None":
        return violates_tier0(raw_target)

    def _runcmd_hits_tier0(self, cmd: str) -> "str | None":
        return runcmd_hits_tier0(cmd)
    ...
    async def execute_step(self, step: Step, context: TaskContext) -> Dict[str, Any]:
        ...
        try:
            # --- TIER-0 / IMMUTABLE CHECK (autonomous path only; Claude-in-loop bypasses worker) ---
            if step.action in ("write_file", "patch_file"):
                reason = self._violates_tier0(step.details.split('\n')[0] if step.details else "")
                if reason:
                    return {
                        "success": False,
                        "error": f"TIER-0 DENY: {reason}. The autonomous loop cannot modify the "
                                 "supervisor/safety/governance layer — that path is the real risk "
                                 "(a confabulated proposal disabling the safety machinery). Changes "
                                 "go through Claude-in-loop + explicit DJ authorization only."
                    }
            # run_command can also write to tier-0 (sed -i / cp / mv / > / tee) — gate it too.
            if step.action == "run_command":
                reason = self._runcmd_hits_tier0(step.details or "")
                if reason:
                    return {
                        "success": False,
                        "error": f"TIER-0 DENY: {reason}. run_command may not mutate the "
                                 "supervisor/safety/governance layer from the autonomous loop."
                    }
            ...
            if step.action == "write_file":
                ...
                # TIER-0 immutability (2026-05-31): the autonomous loop may NEVER write the
                # things that judge / gate / restore / supervise it (verification layer,
                # auto-approve gate, governance, backup/restore, the supervisor). Human +
                # reversibility is the only path. See shared/tier0_guard.py + governance/
                # tier0_immutable.md. This is the enforced floor for Tier 3.
                from shared.tier0 import violates_tier0
                _t0r = violates_tier0(str(path))
                if _t0r:
                    return {
                        "success": False,
                        "error": f"TIER-0 PROTECTED: refusing autonomous write to {path} ({_t0r}). "
                                 "Immutable set — only a human + reversibility may change it.",
                        "tier0_blocked": True,
                    }

                # Write protection (lazy-compliance guard: don't let a 'fix' silently delete the feature)
                if path.exists():
                ...
                # TIER-0 immutability — same guard as write_file (2026-05-31).
                from shared.tier0 import violates_tier0
                _t0r = violates_tier0(str(path))
                if _t0r:
                    return {"success": False,
                            "error": f"TIER-0 PROTECTED: refusing autonomous patch of {path} ({_t0r}). "
                                     "Immutable set — human + reversibility only.",
                            "tier0_blocked": True}
                if not path.exists():
            ...
            # --- SYSTEM COMMANDS ---
                ...
                # Prepend the project venv's bin to PATH so coder-planned commands
                # that invoke bare `python`/`pip` resolve to the venv interpreter
                # (with RouxYou deps), not the system shell — which on this Ubuntu
                # host has only `python3`, never `python`. Without this, every
                # `python -m py_compile ...` verification step false-fails with
                # "/bin/sh: python: not found", causing post-app gate false-BLOCKs
                # on perfectly good changes (2026-05-25).
                # _PROJECT_ROOT is the worker/ dir; the venv lives at the repo
                # root (its parent). Check both so this stays correct if the
                # constant's meaning changes.
                _venv_bin = next(
                    (c for c in (_PROJECT_ROOT / "venv" / "bin",
                                 _PROJECT_ROOT.parent / "venv" / "bin")
                     if c.is_dir()),
                    None,
                )
                _env = dict(os.environ)
                if _venv_bin:
                    _env["PATH"] = f"{_venv_bin}{os.pathsep}{_env.get('PATH', '')}"
                    _env["VIRTUAL_ENV"] = str(_venv_bin.parent)
                result = subprocess.run(step.details, shell=True, capture_output=True,
                                        text=True, timeout=60, cwd=str(work_dir), env=_env)
