...
SERVICES = [
    ...
    },
    {
        ...
        "health": "/health",
        "delay":  5,
        # 2026-06-05 (lever #2 graduation): qwen3-coder needs the GPU arbiter (auto v53 swap),
        # the agentic read-then-edit loop, and the self-edit verify-fix loop. Remove this env
        # to revert the coder to the qwen2.5 baseline behavior.
    ...
    {
        # Roux v53 backend: llama-server (Vulkan, --n-cpu-moe 10, --reasoning-budget 256).
        # 2026-05-30 "the precise way" — ~2.5-3x faster grounded turns than ollama's
        # native engine. Owns ~15GB GPU; starts before orchestrator (companion → llamacpp).
        # GPU frees when the stack is down (tied to lifecycle, not always-on). serve_v53.sh
        # stops any resident ollama roux-v53 first to avoid a GPU collision.
        "name":      "llama-server (Roux v53 / Vulkan)",
        "key":       "llamacpp",
        "script":    None,
        "shell_cmd": ["/home/user/llama-vulkan/serve_v53.sh"],
        "cwd":       None,
        "port":      8090,
        "health":    "/health",
        "delay":     8,
    ...
    },
    {
        ...
        "delay":    5,
        "optional": True,
        # Bind /voice/think on the LAN so HA (homeassistant.local) can reach Roux's brain
        # for the voice puck. Exposes roux_service to the trusted Internal LAN only
        # (IoT VLAN can't reach it directly). TODO: add a shared-token check on /voice/think.
        "env":      {"ROUX_VOICE_HOST": "127.0.0.1"}  # loopback default; set 0.0.0.0 to expose on your LAN,
    },
]
...
def launch_service(svc: dict) -> subprocess.Popen:
    ...
    elif svc.get("shell_cmd"):
        # Non-Python service launched via an explicit command (e.g. llama-server).
        cmd = svc["shell_cmd"]
    else:
    ...
    if IS_WINDOWS:
        ...
        _env = {**os.environ, **svc.get("env", {})} if svc.get("env") else None
...
def main():
    ...
    failed = []
    optional_down = []  # optional services that spawned but never opened their port
                        # (don't fail the launch — but DON'T claim 'all started' either)
    ...
    for svc in services_to_run:
        ...
        while time.time() < deadline:
            ...
            if optional:
                # Spawned but never came up (e.g. crashed on import). Loud + a log
                # hint — don't let it scroll past as a silent yellow blip.
                print(f"  {YELLOW}{msg} (optional — likely CRASHED; check logs/{svc['key']}.log){RESET}")
                optional_down.append(svc["name"])
            else:
    ...
    if failed:
        ...
        if optional_down:
            # Core is up, so keep supervising — but tell the truth: a feature is down.
            print(f"{YELLOW}{BOLD}Core services up — but optional service(s) did NOT come up: "
                  f"{', '.join(optional_down)}{RESET}")
            print(f"{YELLOW}  Not fatal, but a feature is likely down (e.g. voice/8014). "
                  f"Check logs/ for the cause.{RESET}")
        else:
