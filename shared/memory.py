...
class MemorySystem:
        ...
        if MEMORY_FILE.exists():
            try:
                with open(MEMORY_FILE, "r", encoding="utf-8") as f:
                    text = f.read().strip()
                # An empty file (0 bytes — e.g. a write truncated by a crash/reboot, as
                # happened 2026-06-14) is just "no episodes yet", not corruption. Treat it
                # as an empty store instead of logging a scary warning. 2026-06-18.
    ...
    def _persist(self):
        # Atomic write (2026-06-18): write a temp file, fsync, then os.replace onto the
        # real path. open(...,"w") truncates IN PLACE, so a crash between truncate and
        # write leaves a 0-byte file (that's how memory.json died on 2026-06-14). A
        # temp+rename means the live file is only ever the previous-complete or the
        # new-complete copy — never a partial/empty one.
    ...
    def safe_decay(self) -> Dict:
        """run_decay() but snapshot memory.json FIRST — a bad prune is then instantly
        reversible (2026-06-18, the cord-off rollback guarantee). FAIL-SAFE: if the
        pre-decay snapshot can't be written, DO NOT decay (never prune what you can't
        roll back). Keeps the last 20 pre-decay snapshots in memory_predecay/.
        Returns run_decay stats + predecay_backup path + decayed(bool)."""
        backup_path = None
        try:
            if MEMORY_FILE.exists() and MEMORY_FILE.stat().st_size > 0:
                bdir = MEMORY_FILE.parent / "memory_predecay"
                bdir.mkdir(parents=True, exist_ok=True)
                ts = time.strftime("%Y%m%dT%H%M%S", time.localtime())
                backup_path = bdir / f"memory_{ts}.json"
                shutil.copy2(MEMORY_FILE, backup_path)
                for old in sorted(bdir.glob("memory_*.json"))[:-20]:
                    try:
                        old.unlink()
                    except Exception:
                        pass
        except Exception as e:
            print(f"MEMORY: pre-decay snapshot failed ({e}) — SKIPPING decay (won't prune what can't roll back)")
            return {"error": f"pre-decay snapshot failed: {e}", "decayed": False}
        stats = self.run_decay()
        stats["predecay_backup"] = str(backup_path) if backup_path else None
        stats["decayed"] = True
        return stats
