...
def _v53_topic_gap(focuses: List[str], fuel: List[str]) -> Dict:
    """Ask v5.3 whether recent high-relevance fuel reveals ONE recurring on-thesis theme that NO current
    research topic covers. Returns a JSON topic spec {focus, queries, context} or {} if covered. 2026-06-19."""
    V53 = "http://127.0.0.1:8090/v1/chat/completions"
    sys_p = ("You curate research topics for RouxYou — a LOCAL self-modifying AI agent (Qwen3-30B on a 16GB GPU, "
             "LanceDB memory, propose->verify->apply self-improvement loop, Home-Assistant guardian). Given the "
             "CURRENT topic focuses and a list of RECENT high-relevance findings, decide whether the findings CLUSTER "
             "on ONE recurring on-thesis theme that NO current focus addresses. If they do, propose that topic. If the "
             "cluster is plausibly covered by an existing focus, return {}. (Don't over-add — but a genuine uncovered "
             "cluster SHOULD be proposed; that's the point.)")
    usr_p = ("CURRENT TOPIC FOCUSES:\n- " + "\n- ".join(focuses) + "\n\nRECENT FINDINGS:\n- " + "\n- ".join(fuel) +
             '\n\nIf the findings cluster on an uncovered on-thesis theme, return ONE JSON object: '
             '{"focus":"snake_case_name","queries":["query1","query2"],"context":"1-2 sentences: why this matters for '
             'RouxYou and how it fits the local-16GB constraint"}. If the cluster is already covered by an existing '
             'focus, return {}. Output ONLY the JSON object.')
    try:
        r = requests.post(V53, json={"model": "roux-v53",
                                     "messages": [{"role": "system", "content": sys_p}, {"role": "user", "content": usr_p}],
                                     # 4096 (not 1200): v5.3 reasons ~1100 tokens before the JSON; a tight cap
                                     # truncates the real answer → extract_json grabs an echoed fragment. (the
                                     # no-tight-token-cap-on-local-reasoning-models lesson, again.)
        ...
        logger.warning(f"_v53_topic_gap failed: {e}")
        return {}


def observe_topic_gaps(tracker: ProposalTracker) -> List[Proposal]:
    """SEED for roux-proposes-topics (2026-06-19): scan recent high-relevance fuel for a recurring on-thesis
    theme NOT covered by any existing research topic, and if found, emit a research_topic ADD proposal —
    which flows through the gated/validated/reversible pipeline (sync->authored_by_topicmgmt->v5.3 gate->
    _handle_research_topic). Cooldowned (24h scan) + no-pile-up + capped so it can't sprawl or loop."""
    proposals: List[Proposal] = []
    try:
        SCAN_COOLDOWN_H = float(os.environ.get("ROUX_TOPIC_SCAN_COOLDOWN_H", "24"))
        CAP = int(os.environ.get("ROUX_MAX_RESEARCH_TOPICS", "25"))
        now = time.time()
        state_f = STATE_DIR / "topic_gap_scan.json"
        try:
            last = json.loads(state_f.read_text()).get("last_scan_at", 0) if state_f.exists() else 0
        except Exception:
            last = 0
        if now - last < SCAN_COOLDOWN_H * 3600:
            return []
        topics = json.loads((STATE_DIR / "research_topics.json").read_text())
        if not isinstance(topics, list) or len(topics) >= CAP:
            return []
        focuses = [str(t.get("focus", "")) for t in topics if isinstance(t, dict)]
        # no pile-up: don't propose another topic while one is already active
        try:
            act = json.loads((STATE_DIR / "proposals_active.json").read_text())
            act = act if isinstance(act, list) else act.get("proposals", act.get("active", []))
            if any(isinstance(p, dict) and (p.get("category", "").lower() == "research_topic") for p in act):
                return []
        except Exception:
            pass
        proc = sorted((STATE_DIR / "knowledge_inbox" / "processed").glob("*.md"),
                      key=lambda f: f.stat().st_mtime, reverse=True)[:14]
        fuel = []
        for f in proc:
            try:
                fuel.append(f.read_text(encoding="utf-8", errors="replace").splitlines()[0].lstrip("# ").strip()[:90])
            except Exception:
                pass
        if len([x for x in fuel if x]) < 5:
            return []  # not enough recent signal to judge a gap
        # record the scan NOW so we don't re-scan (or re-hit v5.3) for the cooldown window, gap or not
        try:
            state_f.write_text(json.dumps({"last_scan_at": now}))
        except Exception:
            pass
        spec = _v53_topic_gap(focuses, [x for x in fuel if x])
        if not (isinstance(spec, dict) and spec.get("focus")):
            return []
        focus = str(spec["focus"]).strip()
        if not focus or focus.lower() in {f.lower() for f in focuses}:
            return []
        if not (isinstance(spec.get("queries"), list) and spec["queries"] and spec.get("context")):
            return []
        proposals.append(Proposal(
            title=f"Add research topic: {focus}",
            description=f"Coverage gap — recent high-relevance fuel keeps surfacing '{focus}' but no research topic covers it. Proposing it so discovery widens itself (ADD-only; gated + reversible).",
            category="research_topic", priority=4,
            proposed_action="ADD a new research topic. Spec: " + json.dumps(spec),
            evidence="Recurring uncovered theme across recent ingested findings: " + "; ".join([x for x in fuel if x][:6]),
        ))
        logger.info(f"observe_topic_gaps: proposing new research topic '{focus}'")
    except Exception as e:
        logger.warning(f"observe_topic_gaps failed: {e}")
    return proposals
