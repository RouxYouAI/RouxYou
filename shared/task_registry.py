...
class TaskRegistry:
    ...
    def cancel_task(self, task_id: str, reason: str = "") -> bool:
        """Retire a task as CANCELLED (obsolete/superseded) — honest terminal state, NOT 'completed'.
        Returns True if found + cancelled. 2026-06-19 (enables the executor's tasks-handler)."""
        for t in self.tasks:
            if t.id == task_id:
                t.status = TaskStatus.CANCELLED
                if reason and hasattr(t, "blocked_reason"):
                    t.blocked_reason = reason
                self.save()
                return True
        return False
