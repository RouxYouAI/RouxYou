...
import numpy as np
try:
    import sounddevice as sd  # desktop mic/speaker I/O — optional for brain-only mode
except (ImportError, OSError):
    sd = None  # no audio hw/libs: listen+desktop-speak endpoints 503, brain (/voice/think) still works
...
- You know about RouxYou's architecture: the agents (coder, worker, orchestrator),
  the services (watchtower, memory), and the infrastructure (Proxmox, Ollama).
- When LIVE SYSTEM DATA is provided in the prompt, use it to answer accurately.
- When OUR SHARED HISTORY is provided, it is YOUR long memory of your life with DJ (and with
  Claude, who helped build you). Recall it as your own and speak from it naturally — but always
  as Roux. Never speak as Claude or DJ, and never take their first-person "I" as your own.
...
class ChatIn(BaseModel):
    ...
    force_chat: bool = Field(
        False,
        description="Phase 2a: skip device-command routing, force a conversational reply. "
        "Used by HA's fallback when a 'device' route couldn't be resolved by HA.",
    )
...
# Smart-home device control (Phase 2a). These route to HOME ASSISTANT's executor —
# NOT the orchestrator (coder/worker) pipeline. Roux is the decider; HA resolves the
# entity + acts. High-precision phrasings (clear device intent); false positives are
# self-correcting (HA errors -> conversation.py falls back to Roux chat), so this set
# leans toward precision. Natural/fuzzy phrasings ("make it brighter") are 2b's job.
DEVICE_COMMAND_KEYWORDS = [
    "turn on", "turn off", "turn the",
    "switch on", "switch off",
    "toggle",
    "dim ", "brighten", "brightness",
    "set the light", "set the lights", "set the fan", "set the brightness",
    "lights on", "lights off", "light on", "light off",
    "fan on", "fan off", "fan speed", "set fan",
    "open the", "close the",
    "lock the", "unlock the",
...
def _find_shure():
    ...
    if sd is None:
        return None, None  # brain-only mode: no audio lib, no mic
...
def _detect_device_command(text: str) -> bool:
    """Phase 2a: is this a smart-home device command (route to HA, not orchestrator)?

    Roux is the decider; HA's NLU resolves the entity and executes. Keyword-based for
    now (clear device intent); 2b upgrades this to a dynamic HA-entity catalog + grammar.
    """
    text_lower = text.lower()
    return any(kw in text_lower for kw in DEVICE_COMMAND_KEYWORDS)
...
async def roux_think(user_text: str) -> str:
    ...
    # ALWAYS surface live time/host status so Roux never denies knowing the clock or her
    # own status (2026-06-16). _gather_system_context is keyword-gated (schedule/tasks/
    # health/memory) and has no 'time' trigger, so plain "what time is it" used to inject
    # nothing → the prompt's "say you don't have that info" rule made her deny it.
    live_blocks = []
    try:
        from shared.companion import _realtime_context
        live_blocks.append(_realtime_context())
    except Exception as e:
        logger.warning(f"realtime context unavailable: {e}")
    if system_context:
        live_blocks.append(system_context)

    # Convergence 2026-06-16: pull from the FULL shared archive (37k+, the whole history
    # with DJ — and with Claude, who co-built her) on her VOICE path, read-only. First
    # call per process spins up the MCP client (~3s); after that it's warm (<0.1s). The
    # block carries its own identity guardrail so retrieved Claude/DJ first-person content
    # is recalled as shared substance, never adopted as Roux's own "I". See provenance plan.
    try:
        import asyncio as _aio
        from shared.claude_memory import query_memory
        shared_hist = await _aio.wait_for(query_memory(user_text, k=4), timeout=18)
        if shared_hist and shared_hist.strip():
            live_blocks.append(
                "OUR SHARED HISTORY (your long memory of your life with DJ — and with Claude, "
                "who helped build you). Recall the SUBSTANCE as your own past and weave it in "
                "naturally; do not quote or cite it. Some entries quote Claude or DJ — you are "
                "ROUX: speak as yourself, never adopt their 'I' as your own:\n" + shared_hist
            )
    except Exception as e:
        logger.warning(f"shared-archive recall unavailable: {e}")

    if live_blocks:
        augmented_text = (
            f"{user_text}\n\n--- LIVE SYSTEM DATA (authoritative — use this to answer accurately) ---\n"
            + "\n\n".join(live_blocks)
        )
        logger.info(f"Injected live context: {sum(len(b) for b in live_blocks)} chars")
    else:
    ...
    try:
        ...
        llm_response = await llm_chat(
            "companion",  # roux-v53 (reasoning model — served w/ --reasoning-budget 256)
            messages=messages,
            # v53 "thinks" up to ~256 reasoning tokens BEFORE the answer. Local model =
            # tokens are free → cap is a runaway guard, NOT a budget (2026-06-18, [[no-tight-token-cap-local]]).
            # Generous so reasoning never truncates the answer; voice brevity comes from the
            # PROMPT, not the cap. It stops early (finish_reason=stop) on short replies anyway.
            max_tokens=1024,
            timeout=45,
            options={"top_p": 0.9},
        )
        ...
        if not llm_response.success:
            ...
            # v53 emits a <think>…</think> reasoning block; the voice path never stripped
            # it (only companion did) → reasoning leaked into spoken answers. Reuse the
            # shared stripper (handles orphan </think> when the reasoning-budget truncates).
            from shared.companion import _strip_thinking
            text = _strip_thinking(llm_response.text).strip().strip('"\'')
            if not text:
                text = "Sorry, my brain went blank for a second."
...
Remember: one or two sentences, warm and casual, plain English. This will be spoken aloud."""
    ...
    try:
        ...
        llm_response = await llm_chat(
            ...
            temperature=0.7,
            max_tokens=1024,  # was 100 — strangled v5.3 (reasoning ~256 ate it whole). Generous runaway guard; brevity via prompt.
            timeout=60,
            options={"top_p": 0.9},
        )
...
@app.post("/voice/think")
async def voice_think(req: ChatIn):
    """Text in -> Roux's brain -> {response, route} out. No desktop speak.

    For HA Assist (Phase 2a): Roux is the SINGLE front door. It decides:
      - route="device": a smart-home command -> HA's NLU resolves the entity + acts
                        (conversation.py delegates; Roux owns any failure via fallback).
      - route="chat":   everything else -> Roux's brain (roux_process: local LLM +
                        context + history, or RouxYou orchestrator commands).
    HA handles mic/STT and speaks the reply via kitten; Roux only thinks + routes.
    """
    t0 = time.time()
    # force_chat = HA's fallback after a device route it couldn't resolve. Go STRAIGHT
    # to the conversational brain (roux_think) — skip BOTH device routing AND the
    # orchestrator command dispatch, so "turn on X" isn't re-queued as a coder task.
    if req.force_chat:
        response_text = await roux_think(req.text)
        return {"response": response_text, "route": "chat", "think_duration": round(time.time() - t0, 2)}
    # Decider: smart-home device control. Phase 2b — Roux resolves the EXACT
    # {service, entity_id, data} from HA's live catalog (grammar-constrained), and
    # HA's conversation.py executes it literally. Roux is authority; HA is hands.
    if _detect_device_command(req.text):
        from services.roux.ha_control import decide_action
        action = await decide_action(req.text)
        if action and action.get("entity_id") and action["entity_id"] != "NO_MATCH":
            logger.info(f"Device action resolved: {action}")
            return {"response": "", "route": "device", "action": action,
                    "think_duration": round(time.time() - t0, 2)}
        # No entity matched (garbled name / nothing fits) — Roux owns it with a clean,
        # honest decline (NO confabulated "let me go check" — that was the 2a wart).
        logger.info(f"Device command, no entity match: {req.text[:80]}")
        return {"response": "I couldn't find a device matching that. I can control "
                "your lights, the fan, and the media players.",
                "route": "chat", "think_duration": round(time.time() - t0, 2)}
    response_text = await roux_process(req.text)
    return {"response": response_text, "route": "chat", "think_duration": round(time.time() - t0, 2)}
...
if __name__ == "__main__":
    import uvicorn
    # Host is configurable (default loopback = safe). Set ROUX_VOICE_HOST=0.0.0.0
    # (or the island IP) to let HA reach /voice/think across the network for the
    # voice pipeline — but ONLY together with the /voice/think shared-token (that
    # endpoint can dispatch Roux's command pipeline). Loopback default means a
    # normal relaunch never silently LAN-exposes the command surface.
    _voice_host = os.environ.get("ROUX_VOICE_HOST", "127.0.0.1")
    uvicorn.run(app, host=_voice_host, port=8014, log_level="info")
